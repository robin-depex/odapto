...
if($type == "board")
{
	...
	if($fianl_result['result'] == "TRUE"){
		...
		<a style="color:#fff;margin-top:3px;min-height: 30px;line-height: 24px;font-size:15px;display: block;margin-bottom:3px;text-decoration: none" href="javascript:void(0)" onclick="return addMembers(this.id)" id="<?php echo $id."_".$bid ?>"><?php echo $name; ?> (<?php echo $emailid;?>)</a> </li>
		<?php	
		}
		?>
		</ul>
	...
	if($fianl_result['result'] == "TRUE"){
		...
		<a style="color:#fff;margin-top:3px;min-height: 30px;line-height: 24px;font-size:15px;display: block;margin-left: 10px; margin-bottom:3px;text-decoration: none" href="javascript:void(0)" onclick="return addTeamMembers(this.id)" id="<?php echo $id."_".$bid ?>"><?php echo $name; ?> (<?php echo $emailid; ?>)</a></li>
		<?php	
		}
		?>
		</ul>
...
<script type="text/javascript">
	...
	function addTeamMembers(elem){
		...
		$.post('add-existing-members-team.php', {data: id}, function(response) {
			if(response==0){
            $('#useradderror').html("User Already Invited");
