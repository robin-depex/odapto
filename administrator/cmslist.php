...
<tr>
<td colspan="2" class="titel">All CMS List</td>
...
<a href="<?php echo ADMIN_URL;?>index.php?page=addcms" style="font-size:12px; color:#FFFFFF; text-decoration:none;"><span class="buttons">Add Page</span></a></td>
</tr>
<tr>
<td colspan="4">
...
<td align="left" width="50%" >Cms Title</td>
...
$sqlPaging = "SELECT  * FROM ".TBL_PREFIX."_pages ";
...
$sql1 = "SELECT * FROM ".TBL_PREFIX."_pages ORDER BY id DESC ".$arrays['limit'];
...
<td class="txtmain" align="left" valign="top"><?php echo stripslashes($rows->page_title);?></td>
<td align="center" valign="top" class="txtmain"><strong><?php echo DateFormat($rows->adddate); ?></strong></td>
<td align="center" valign="top" ><a href="<?php echo ADMIN_URL;?>index.php?page=addcms&id=<?php echo intval($rows->id); ?>"><img src="images/edit-icon.png" border="0" title="Edit" /></a> &nbsp;<img src="images/<?php echo ($rows->status==1)?'icon-active.gif':'inactive.png';?>" border="0" title="<?php echo ($rows->status==1)?'Published':'Unpublished';?>" /></td>
</tr>

<?php $num++; } ?>

<tr>
  <td colspan="5" class="dark_red"  align="right">
...
<tr>
<td colspan="5" class="dark_red" valign="top"><table align="center">
<tbody>
<tr>
<td align="center">
...
echo '&nbsp;<a href="'.ADMIN_URL.'index.php?page=cms&pages='.$arrays['previous'].'"><span style=" border: 1px solid #C3C3C3; color: #1122CC; font-family: Arial,Helvetica,sans-serif;font-size: 12px; font-weight: bold; padding: 0 3px; margin-right:11px;">Previous</span></a>';

foreach($arrays['pages'] as $numPages){
echo '<a href="'.ADMIN_URL.'index.php?page=cms&pages='.$numPages.'">';
?>
...
echo '&nbsp;<a href="'.ADMIN_URL.'index.php?page=cms&pages='.$arrays['next'].'"><span style=" border: 1px solid #C3C3C3; color: #1122CC; font-family: Arial,Helvetica,sans-serif;font-size: 12px; font-weight: bold; padding: 0 3px; ">Next</span></a>';
echo "&nbsp;&nbsp;&nbsp;&nbsp;";
   ...
   function del_prompt(frmobj)
		...
		var comb = frmobj.action2.value;
			...
			if(comb=="Delete"){
				...
				{
					frmobj.action = "status/cms_status.php?action=Delete";
					frmobj.submit();
				}
				else{ 
				return false;
					...
					frmobj.action="status/cms_status.php?action=Deactivate";
			frmobj.submit();
		}
		else if(comb=="Activate"){			
			frmobj.action="status/cms_status.php?action=Activate";
			frmobj.submit();
		}
