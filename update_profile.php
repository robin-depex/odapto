...
if(isset($_REQUEST['token_ses'])){
	if($_REQUEST['token_ses'] == $_SESSION['Tocken']){
		...
		$where_u=array('ID' => $uid);
		$data_u=array('Full_Name' => $fullname);
		$update = $db->update("tbl_users",$data_u,$where_u);
		
		if($update){
			$response = json_encode(array("result"=>"TRUE","message"=>"Profile Updated"));	
		}else{
...
if($_POST['profile']==="profile"){
		...
		$data = array("meta_value"=>$initials);
		$con = array("meta_key"=>"initials","user_id"=>$uid);
		$update = $db->update("tbl_usermeta",$data,$con);
...
if (isset($_FILES['file']['name'])) {
	...
	$password=md5(date('ymdhis').$_FILES['file']['name']).'.png';
	$uid = $_SESSION['sess_login_id'];
	$data = array("profile_pics"=>$password,'profile_pic_type'=>'file');
	$update = $db->SingleRecordUpdate("tbl_users",$data,$uid);

    if (0 < $_FILES['file']['error']) {
        echo 'Error during file upload' . $_FILES['file']['error'];
    } else {
        if (file_exists('user_profile_Image/' . $password)) {
           
        } else {
            move_uploaded_file($_FILES['file']['tmp_name'], 'user_profile_Image/' .$password);
            $results = array(
    			'result'=>'TRUE',
    			'message'=> $password
    	    );
