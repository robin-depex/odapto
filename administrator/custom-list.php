...
<tr>
<td colspan="2" class="titel">All Custom Field</td>
...
<a href="<?php echo ADMIN_URL;?>index.php?page=addfield" style="font-size:12px; color:#FFFFFF; text-decoration:none;"><span class="buttons">Add Custom Field</span></a></td>
</tr>
<tr>
<td colspan="4">
...
<td align="left" width="23%">Category Name </td>
<td align="center" width="23%">Type Name </td>
<td align="center" width="23%">Type URL </td>
...
$sqlPaging = "SELECT  ID FROM ".TBL_PREFIX."_custom_field ORDER BY ID DESC ";
...
$sql1 = "SELECT * FROM ".TBL_PREFIX."_custom_field ORDER BY ID DESC ".$arrays['limit'];
...
<td align="left" valign="top" class="txtmain"><?php echo GetFieldValue('Category_Name','tbl_category','ID',$rows->Category_ID);?></td>
<td align="center" valign="top" class="txtmain"><?php echo addslashes($rows->Type_Name);?></td>
<td align="center" valign="top" class="txtmain"><?php echo addslashes($rows->Type_Url);?></td>
<td align="center" valign="top" class="txtmain"><strong><?php echo DateFormat($rows->AddDate); ?></strong></td>
<td align="center" valign="top" ><a href="<?php echo ADMIN_URL;?>index.php?page=addfield&id=<?php echo intval($rows->ID); ?>"><img src="images/edit-icon.png" border="0" title="Edit" /></a> &nbsp;<img src="images/<?php echo ($rows->status==1)?'icon-active.gif':'inactive.png';?>" border="0" title="<?php echo ($rows->status==1)?'Published':'Unpublished';?>" /></td>
</tr>
...
echo '&nbsp;<a href="'.ADMIN_URL.'index.php?page=customfield&pages='.$arrays['previous'].'"><span style=" border: 1px solid #C3C3C3; color: #1122CC; font-family: Arial,Helvetica,sans-serif;font-size: 12px; font-weight: bold; padding: 0 3px; margin-right:11px;">Previous</span></a>';

foreach($arrays['pages'] as $numPages){
echo '<a href="'.ADMIN_URL.'index.php?page=customfield&pages='.$numPages.'">';
?>
...
echo '&nbsp;<a href="'.ADMIN_URL.'index.php?page=customfield&pages='.$arrays['next'].'"><span style=" border: 1px solid #C3C3C3; color: #1122CC; font-family: Arial,Helvetica,sans-serif;font-size: 12px; font-weight: bold; padding: 0 3px; ">Next</span></a>';
echo "&nbsp;&nbsp;&nbsp;&nbsp;";
   ...
   function del_prompt(frmobj)
		...
		var comb = frmobj.action2.value;
			...
			if(comb=="Delete"){
				...
				{
					frmobj.action = "status/custom-field-status.php?action=Delete";
					frmobj.submit();
				}
				else{ 
				return false;
					...
					frmobj.action="status/custom-field-status.php?action=Deactivate";
			frmobj.submit();
		}
		else if(comb=="Activate"){			
			frmobj.action="status/custom-field-status.php?action=Activate";
			frmobj.submit();
		}
