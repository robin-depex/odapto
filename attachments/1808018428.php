	...
	public function ingredientsList()
	{
	    ...
	    if (is_array($rows) && count($rows)>=1){
	    	foreach ($rows as $val) {    	     	    		
	    		...
	    		$date=Yii::app()->functions->translateDate($date);*/
	    		$date=FormatDateTime($val['date_created']);
	    		 $merchant_data=Yii::app()->functions->getbyid('mt_merchant','merchant_id',$val['merchant_id']);
    	    		
	    		$feed_data['aaData'][]=array(
	    		  $chk,$val['ingredients_name'].$option,$merchant_data[0]['restaurant_name'],
...
$this->msg=t("We have sent bank information instruction to your email")." :$merchant_email";
	...
	public function driverList()
	{
		...
		$slug=$this->data['slug'];
		$slug1=$this->data['slug1'];
		
		$stmt = "Select * FROM {{driver}} ORDER BY id DESC";
		...
		if ( $res=$this->rst($stmt)){		
			...
			foreach ($res as $val) {	
					//$date=date('M d,Y G:i:s',strtotime($val['create_date']));
					//$date=Yii::app()->functions->translateDate($date);
					$date=FormatDateTime($val['create_date']);
					
					$driver_order_stmt = "Select * FROM {{order}} WHERE driver_id = '".$val['id']."'";
		//	echo $driver_order_stmt;
				$driver_order_res=$this->rst($driver_order_stmt);
				if(empty($driver_order_res)){
				    	$count_order = 0;
				}else{
				    	$count_order = count($driver_order_res);
				}
		
			
					
					$action="<div class=\"options\">
					<a href=\"$slug/id/$val[id]\" >".Yii::t("default","Edit")."</a>
    	    		</div>";
    	    		
    	    		
    if($count_order!=0){
$action1="<div>
					<a href=\"$slug1/id/$val[id]\" >".$count_order."</a>
    	    		</div>";
}else{
	$action1=$count_order;
    	    			...
    	    			$img=Yii::app()->request->baseUrl."/upload/$val[profile]";
    	    		    $photo="<img class=\"uk-thumbnail uk-thumbnail-mini\" src=\"$img\" >";	
    	    		} else $photo='';
					
					if ($val['status']=="0"){
					   $status = '<div class=\"uk-badge uk-badge-danger\">InActive</div>';
					}  elseif ($val['status']=="1"){
						$status = '<div class=\"uk-badge uk-badge-success\">Active</div>';
					}	

					if ($val['is_working']=="0"){
					   $is_working = '<div class=\"uk-badge uk-badge-danger\">No</div>';
					}  elseif ($val['is_working']=="1"){
						$isworking = '<div class=\"uk-badge uk-badge-success\">Yes</div>';
					}			
					...
					$aa_access=Yii::app()->functions->AAccess();
						...
						$feed_data['aaData'][]=array(
						  $val['id'],stripslashes($val['first_name'].' '.$val['last_name']).$action,
						  $val['email'],
						  $val['phone'],
						  $photo,
						  $date,$is_working,
