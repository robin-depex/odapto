...
if(count($arr)>0){
	...
	if($Submit=="Delete")
	{$sql="delete from ".TBL_PREFIX."_users where ID in ($str_rest_refs)"; 
		$obj->execute($sql);
	...
	{	
		$sql="update ".TBL_PREFIX."_users set status=1 where ID in ($str_rest_refs)";	
		$obj->execute($sql);
	...
	{
		$sql="update ".TBL_PREFIX."_users set status=0 where ID in ($str_rest_refs)";
		$obj->execute($sql);
	...
	$objCommon->wtRedirect("../index.php?page=user");	
	}
	$objCommon->wtRedirect("../index.php?page=user");
?>