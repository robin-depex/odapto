...
if(isset($_GET['id'])){
   ...
   // $description = $data['description'];
    $status = $data['status'];
    $h1 = 'Update Template Category';
}else{
    $h1 = 'Add New Template Category';
	$uid = '';
    $btn_title = "Create";
    $btn_id = "register";
    $cat_name = '';
    ...
    ?>		
		...
		<div class="form-group">
            <div class="col-sm-2"><label class="control-label">Active</label></div>
				...
				<input name="status" id="status" type="radio" value="1" <?php echo ($status == 1)? 'checked': '';?>>
				
				<span class="circle"></span><span class="check"></span> Yes </label></div>
				<div class="radio col-sm-6"><label>
				
				<input name="status" id="status" type="radio" value="0"  <?php echo ($status == 0)? 'checked': '';?>>
...
var obj = jQuery.parseJSON(rel);
	if(obj.result=="TRUE"){
		//alert(obj.message);
		window.location.href = "https://odapto.com/admin/dashboard.php?page=temp_list_cat";
...
if(obj.result=="TRUE"){
//alert(obj.message);
var url = "<?php echo "https://odapto.com/admin/dashboard.php?page=temp_list_cat&id=".$uid; ?>";
