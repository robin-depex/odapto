...
class SimpleImage {
   ...
   function resize($width,$height) {
	...
	function checkImageFile($imgfile){
	$imgExtension = array('jpg','jpe','jpeg','gif','png');
	 $image_name = pathinfo($imgfile);
	  $extension = strtolower($image_name['extension']);
		...
		return 0;
        	...
        	function checkPdfFile($pdffile){
	$pdfExtension = array('pdf');
	 $pdf_name = pathinfo($pdffile);
	  $extension = strtolower($pdf_name['extension']);
