...
function database_read($orderId)
{
	...
	$sql_updt = "select payment_status from appointment WHERE order_id='".$orderId."'";
	$sql_query = mysqli_query($con, $sql_updt);
	$data = mysqli_fetch_object($sql_query);
	$status = $data->payment_status;
    return $status ? $status : "unknown order Order is not Completed.";
}
