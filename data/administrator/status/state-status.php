...
if(count($arr)>0){
	...
	if($Submit=="Delete")
	{$sql="delete from ".TBL_PREFIX."_state where ID in ($str_rest_refs)"; 
		$obj->execute($sql);
	...
	{	
		$sql="update ".TBL_PREFIX."_state set status=1 where ID in ($str_rest_refs)";	
		$obj->execute($sql);
	...
	{
		$sql="update ".TBL_PREFIX."_state set status=0 where ID in ($str_rest_refs)";
		$obj->execute($sql);
	...
	$objCommon->wtRedirect("../index.php?page=state");	
	}
	$objCommon->wtRedirect("../index.php?page=state");
?>