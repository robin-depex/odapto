...
<tr>
<td colspan="2" class="titel">All Category List</td>
...
<a href="<?php echo ADMIN_URL;?>index.php?page=category-action" style="font-size:12px; color:#FFFFFF; text-decoration:none;"><span class="buttons">Add Category</span></a></td>
</tr>
<tr>
<td colspan="4">
...
<td align="left" width="22%" >Parent Name </td>
<td align="left" width="25%">Category Name </td>
<td align="center" width="25%">Category URL</td>
...
$sqlPaging = "SELECT  ID FROM ".TBL_PREFIX."_category ORDER BY ID DESC ";
...
$sql1 = "SELECT * FROM ".TBL_PREFIX."_category ORDER BY ID DESC ".$arrays['limit'];
...
<td class="txtmain" align="left" valign="top">
<?php if($rows->Parent_ID == 0){echo '<strong>Parent</strong>';}else {?>

<?php echo GetFieldValue('Category_Name',TBL_PREFIX."_category",'ID',$rows->Parent_ID); }?></td>
<td align="left" valign="top" class="txtmain"><?php echo stripslashes($rows->Category_Name);?></td>
<td align="center" valign="top" class="txtmain"><?php echo addslashes($rows->Category_Url);?></td>
<td align="center" valign="top" class="txtmain"><strong><?php echo DateFormat($rows->AddDate); ?></strong></td>
<td align="center" valign="top" ><a href="<?php echo ADMIN_URL;?>index.php?page=category-action&id=<?php echo intval($rows->ID); ?>"><img src="images/edit-icon.png" border="0" title="Edit" /></a> &nbsp;<img src="images/<?php echo ($rows->status==1)?'icon-active.gif':'inactive.png';?>" border="0" title="<?php echo ($rows->status==1)?'Published':'Unpublished';?>" /></td>
</tr>
...
echo '&nbsp;<a href="'.ADMIN_URL.'index.php?page=category&pages='.$arrays['previous'].'"><span style=" border: 1px solid #C3C3C3; color: #1122CC; font-family: Arial,Helvetica,sans-serif;font-size: 12px; font-weight: bold; padding: 0 3px; margin-right:11px;">Previous</span></a>';

foreach($arrays['pages'] as $numPages){
echo '<a href="'.ADMIN_URL.'index.php?page=category&pages='.$numPages.'">';
?>
...
echo '&nbsp;<a href="'.ADMIN_URL.'index.php?page=category&pages='.$arrays['next'].'"><span style=" border: 1px solid #C3C3C3; color: #1122CC; font-family: Arial,Helvetica,sans-serif;font-size: 12px; font-weight: bold; padding: 0 3px; ">Next</span></a>';
echo "&nbsp;&nbsp;&nbsp;&nbsp;";
   ...
   function del_prompt(frmobj)
		...
		var comb = frmobj.action2.value;
			...
			if(comb=="Delete"){
				...
				{
					frmobj.action = "status/category-status.php?action=Delete";
					frmobj.submit();
				}
				else{ 
				return false;
					...
					frmobj.action="status/category-status.php?action=Deactivate";
			frmobj.submit();
		}
		else if(comb=="Activate"){			
			frmobj.action="status/category-status.php?action=Activate";
			frmobj.submit();
		}
